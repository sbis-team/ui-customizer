// ==UserScript==
// @name          SBIS UI-Customizer v2.0.0-betta
// @namespace     SBIS
// @version       2.0.0-betta
// @author        Новожилов И. А.
// @description   Пользовательская настройка интерфейса сайта online.sbis.ru
// @homepage      https://github.com/sbis-team/ui-customizer
// @icon          data:image/png;base64,
// @icon64        data:image/png;base64,
// @supportURL    https://github.com/sbis-team/ui-customizer/issues
// @include       *online.sbis.ru/*
// @exclude       *online.sbis.ru/obj/*
// @exclude       *online.sbis.ru/shared/*
// @exclude       *online.sbis.ru/docview/*
// @exclude       *online.sbis.ru/auth/*
// @exclude       *online.sbis.ru/webrtc/*
// @exclude       *online.sbis.ru/carry.html*
// @exclude       *online.sbis.ru/presto.html*
// @exclude       *online.sbis.ru/booking.html*
// @run-at        document-start
// @noframes
// ==/UserScript==
(function () {
  'use strict';

  const core = document.createElement('script');
  core.id = 'SBIS-UI-Customizer-V2--core';
  core.type = 'text/javascript';
  core.src = 'https://cdn.jsdelivr.net/gh/sbis-team/ui-customizer@candidate-v2/src/core.js';

  const container = document.createElement('userscript');
  container.id = 'SBIS-UI-Customizer-V2';
  container.setAttribute('data-vdomignore', 'true');
  container.appendChild(core);

  document.getElementsByTagName('html')[0].appendChild(container);

})();
